...
To change the capacity of your Glue job, you will need to set the following __glue_job_worker_type__ and __number_of_workers_for_glue_job__ variables.
...
or performing complex transformations.
  ...
  - __glue_job_worker_type__ (optional): Worker type for Glue job (see [AWS documentation on 'Worker types][glue-worker-types]'). The default is __Standard__. This can either be set as "Standard", "G.1X" or "G.2X". 
